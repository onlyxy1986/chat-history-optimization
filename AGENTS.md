...
- `manifest.json` `generate_interceptor` must match the global function `replaceChatHistoryWithDetailsV2` (defined on `globalThis` in `config/engine.js`). Renaming it breaks ST's generation pipeline.
