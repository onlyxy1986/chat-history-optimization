...
4. **分层摘要（v2.21.0 起，替代逐条目二级摘要 + 混合召回）**：L0 历程条目 → L1 天摘要（一 key 一条）→ L(k≥2) 连续 `hierFanin` 个同层节点合并，纯文本摘要持久化在 `chat_metadata`，父节点以子文本哈希校验。超预算时从最旧侧压实：先尽量把最旧侧完整的同层组并成父节点（L1→L2→L3…），无完整组可并才折最旧 L0→L1，都不行才丢最旧天（高层永远在远端；最大化保留较新天的 L0 原文），token 够用时零压缩；发送前永不等 LLM，缺失部分用原文兜底。
...
7. **分层折叠路径**（触发且 `runFold` 时，见 `planFoldSlots`）：
     ...
     - `midBudget = contentLimit - tailTok`；中段按天分组（首现顺序，天为原子单位），从最旧侧折叠：
       - **折叠优先序（从最旧侧压实）**：① 尽量把最旧侧完整的同层组并成父节点（`fanin` 个 L1 → L2、`fanin` 个 L2 → L3…，逐层上升；span 必须整体完全落在中段内，防正文重复）；② 没有可合并的完整同层组，才把最旧 L0 折成有效 L1（须完全落在中段内；部分被正文覆盖的天永不折叠）。估算阶段与精确计数兜底保持同一优先序：老天先压紧，让预算优先由高层摘要腾出，从而把较新的天尽量留成 L0 原文。
      - **精确计数兜底（v2.23.3 修复估算偏乐观丢整天）**：估算口径（`EST_CHARS_PER_TOKEN=1.5`）与真实 tokenizer 不一致时可能"该折的没折"（如只超 1k 却要丢 5k 的整天）。故折叠后渲染做 `getTokenCountAsync` 精确计数；仍超则按同一优先序兜底：①上层合并、②最旧可折叠 L0→L1、③都不可行才从最旧槽位逐个丢弃（被合并吞掉的天不占预算，不参与丢弃；整跨度父槽位被丢即整段消失），直至 ≤ midBudget（无次数上限，保证硬上限）。`spanFullyInside` 只要求 span 内实际存在的天全部落在中段内，数字断层直接跳过（v2.23.1 修复：此前缺一天即整父永不可用，非连续时间线白生成）。
     - 发送前永不等 LLM：只读 `SubSummary.getCoverage()` 快照，缺失摘要的天保持原文（折叠优先序②跳过），折叠抛错 → 回退全量中段。
...
- 摘要伪装成与 L0 同形的条目，复用 `renderJourneyMarkdown`，三段**分开渲染后拼接**（不混在一次调用里，防同天 L1/L0 被错误合并）：
  - L1 → `{天数:"第X天", 时间段:"全天", 地点:当天出现次数 top N 地点, 历程:摘要原文}`；
  - L(k≥2) → `{天数:"第X天~第Y天", 时间段:"多日", 地点:覆盖范围内出现次数 top N 地点, 历程:摘要原文}`。
  - 地点字段不全量拼接：按时间范围内出现次数取前 `Constants.SUMMARY_LOCATIONS_MAX`（默认 5）个地点（次数并列按首现顺序），用 `,` 连接，末尾追加 `等地点`（如 `A,B,C等地点`），避免跨天父摘要把所有地点铺开撑大 token。跨天父摘要按 span 内各天 `locationCounts` 求和后取 top。
...
| 折叠按天原子 + 从最旧侧压实 | 天不可拆（整天要么原文要么摘要要么丢弃）；只压缩最旧的天，高层永远在远端；优先把最旧侧完整同层组并成父节点（L1→L2→L3…），无完整组可并才折 L0→L1，最大化保留较新天的 L0 原文 |
...
| 折叠估算 1.5 字符/token + 精确计数兜底 | 估算优先保速度（`getTokenCountAsync` 调用重，尽量少调）；估算与精确计数同一优先序（①上层合并 ②L0→L1 ③丢最旧槽位），精确计数**无次数上限**地兜底至 ≤ midBudget |
...
- 构造多天假聊天（默认一楼层一条，B/C 场景每天 3 条），跑 8 个场景（预算相对全量动态校准）：
  ...
  - **C**（上层合并）：20 天 fanin 3 → 生成 6 个 L2；深压预算 → 从最旧侧压实（L2→L3 逐层上升），实际渲染槽位从旧到新层级非递增（高层离尾远），跨天伪条目含 `|多日|`，最终不超限。
