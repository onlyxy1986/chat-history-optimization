...
const defaultSettings = {
    ...
    "角色信息": { // {{user}}和其他NPC的信息记录
        "{{角色名}}": { //角色名(中文)
            ...
            "当前状态": "[第X天][时间][地点]角色可观测的具体状态，包括：姿势、动作、生理反应、环境交互（避免主观形容词，用行为表现代替情绪）",  // 示例："[第2天][10:15]双腿被皮带固定于沙发扶手，全身痉挛，阴道持续收缩，发出断续尖叫，眼角有泪"
            "物品": { // 角色长期使用或主要用途的物品，排除一次性或临时物品，需随当前回复增减物品
                // "物品1":{ "数量": 1, "用途": "用途描述**复制原文**" }
            },
            "技能": { // 角色的技能记录，随当前回复新增/调整
                // "技能1":{ "等级": 1, "效果": "效果描述**复制原文**" }
            },
    ...
    "信息记录": [// 记录回复中的行为结果、伏笔、伏笔收尾、要求、规则、线索、通知、说明等会对后文内容产生持续影响的关键信息，涉及**数字**的信息要保留数字及其相关信息
