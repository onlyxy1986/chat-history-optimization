    ...
    "characters": { // {{user}}和其他NPC的信息记录
        "character_name": { // 角色名
            ...
            "persistent_state": {  // 【持续状态】事件引发的较长时间身体状态改变（持续几分钟以上至永久）
                // **所有(无论是否在当前回复中提及)角色**都需根据当前时间更新状态的[持续时间](可预估),持续时间归零后移除该状态
            ...
            "clothing": { // 【穿戴层】可随时穿上/脱下的物品，包含：衣物/饰品/玩具/电子设备
                // **所有(无论是否在当前回复中提及)角色**都需在日期变换时通过置空字符串来清空的clothing 例如: "下身": ""
                // 提取角色被提及的所有身体部位着装信息, 格式 "部位":"着装描述"，如佩戴饰物或者玩具也需记录
                // 示例1: "下身": "黑色西裤，黑色丝袜，黑色内裤",
                // 示例2: "乳头": "黑色金属乳环，银色乳夹"
                // 示例3: "屁眼": "粗大的肛塞"
            },
            // **所有(无论是否在当前回复中提及)角色**都需根据next_action更新current_state,并安排新的next_action
