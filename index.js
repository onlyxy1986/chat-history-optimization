...
const defaultSettings = {
    ...
    "characters": { // {{user}}和其他NPC的信息记录
        "character_name": { // 角色名
            ...
            "dynamic_state": { // 【实时反应】事件触发的即时变化，包含：生理反应/情绪外显/穿戴引发的身体状态
                // 提取角色被提及的所有外貌和部位的**动态特征**或**实时状态**描述（随时间/事件变化的特征）, 填充时自选格式:
                // 格式1: "部位1": "状态描述"
                // 格式2: "部位2": { "状态属性1": "描述", "状态属性2": "描述" }
                // 示例1: "脸颊": "泛着不自然的潮红"
                // 示例2: "呼吸": {"频率": "急促", "特征": "带着甜腻的喘息声"}
                // 示例3: "阴茎": {"状态": "勃起", "特征": "前端渗出透明液体", "尺寸变化": "增长至18cm"}
                // 示例4: "全身": {"肌肉状态": "紧绷", "出汗情况": "细密汗珠覆盖背部"}
            },
            "clothing": { // 【穿戴层】可随时穿上/脱下的物品，包含：衣物/饰品/玩具/电子设备
                // 提取角色被提及的所有身体部位着装信息, 格式 "部位":"着装描述"，如佩戴饰物或者玩具也需记录
                // 示例1: "下身": "黑色西裤，黑色丝袜，黑色内裤",
                // 示例2: "乳头": "黄豆大小，佩戴黑色金属乳环"
                // 示例3: "屁眼": "粉红色，塞入粗大的肛塞"
            },
            // **所有(无论是否在当前回复中出现)角色**都需根据next_action更新current_state,并安排新的next_action
            "current_state": "角色当前的身心状态及环境交互（心理状态+生理反应+位置/情境）",  // 示例："心跳加速地蜷缩在沙发角落，双腿无意识地磨蹭"
            "next_action": "角色接下来计划执行或预期发生的具体行为",  // 示例："准备在对方靠近时假装滑倒"
